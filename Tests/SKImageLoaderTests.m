...
{
    [loader loadImageFromUrl:[NSURL URLWithString:@"http://image.spreadshirt.net/image-server/v1/compositions/25386428/views/1"] withWidth:[NSNumber numberWithInt:11] onSuccess:^(UIImage *image) {
        GHAssertNotNil(image, @"image with correct dimensions should load");
    } onFailure:^ (NSError *error) {
        GHFail(@"There should be no error");
    }];
    
    [loader loadImageFromUrl:[NSURL URLWithString:@"http://image.spreadshirt.net/image-server/v1/compositions/25386428/views/1"] withWidth:[NSNumber numberWithInt:12] onSuccess:^(UIImage *image) {
    ...
    [loader loadImageFromUrl:[NSURL URLWithString:@"foo"] withWidth:[NSNumber numberWithInt:11] onSuccess:^(UIImage *image) {
        GHFail(@"Should not succeed");
    } onFailure:^ (NSError *error) {
