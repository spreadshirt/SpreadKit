//
//  Constants.h
//  SpreadKit
//
//  Created by Sebastian Marr on 17.07.12.
//
//

#import <Foundation/Foundation.h>

FOUNDATION_EXPORT NSString *const SPErrorDomain;
FOUNDATION_EXPORT NSInteger const SPURLInferError;
FOUNDATION_EXPORT NSInteger const SPImageUploadFailedError;
FOUNDATION_EXPORT NSInteger const SPPostFailedError;
FOUNDATION_EXPORT NSInteger const SPURLMissingError;
FOUNDATION_EXPORT NSString *const SPErrorMessageKey;