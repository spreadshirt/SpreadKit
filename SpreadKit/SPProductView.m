...
@implementation SPProductView

- (id)initWithProduct:(SPProduct *)theProduct andFrame:(CGRect)frame
{
...
- (void)commonInit
{
    ...
    self.appearanceChooserView = [[SPAppearanceChooserView alloc] initWithFrame:appearanceChooserFrame];
    ...
    self.productTypeChooserView = [[SPProductTypeChooserView alloc] initWithFrame:productTypeChooserFrame];
...
- (void)setProductType:(SPProductType *)theProductType
{
...
- (void)setProduct:(SPProduct *)theProduct
{
    _product = theProduct;
    self.productType = _product.productType;
    
    for (SPProductConfiguration *conf in _product.configurations) {
        
        SPViewMap *map = [self.view viewMapByPrintAreaId:conf.printArea.identifier];
        ...
        SPProductConfigurationView *configurationView =[[SPProductConfigurationView alloc] initWithProductConfiguration:conf andFrame:confFrame];
...
- (void) loadProductTypeImage {
    ...
    [[[SPImageLoader alloc] init] loadImageFromUrl:viewImageURL  withSize:self.bounds.size andAppearanceId:self.product.appearance.identifier completion:^(UIImage *image, NSURL *imageUrl, NSError *error) {
